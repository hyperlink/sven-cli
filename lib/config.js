		...
		.done(function(config){
			...
			if (!options.add && !options.remove) {
				if (Object.keys(config).length) {
					console.log(highlight(JSON.stringify(config, null, Array(4).join(' ')), {json:true}))
				} else {
					promptToAddNewKey()
				}
			}
...
function validateSvnPath(value) {
	if (value.trim() === '') {
		return false
	}
	var done = this.async()
	utils.promiseSvn('list', value).then(function ok(){
		done(true)
	}, function error(){
		done('Invalid SVN location: '+value)
	})
}
