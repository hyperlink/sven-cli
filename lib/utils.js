...
module.exports = {
	...
	getProjectConfigFor     : getProjectConfigFor,
...
function getProjectConfigFor(url) {
	return getConfig().then(function(config) {
		for(var i in config) {
			if (~url.indexOf(config[i].trunk) || ~url.indexOf(config[i].branch)) {
				return config[i]
			}
		}
		return null
		...
		return cwdBranch().then(function(result){
			return getProjectConfigFor(result.url)
				.then(function(result){
					...
					return promptForProjectInfo(message)
				})
		})
	}
	return promptForProjectInfo(message)
...
function promptForProjectInfo(message) {
	var projectsConfig
...
function getBranchLog(branch, config) {
	var defaultConfig = {'stop-on-copy': true, verbose: true}
	if (config == null) {
	...
	return promiseSvn('log', branch, defaultConfig).then(function(output) {
		// console.log(output)
