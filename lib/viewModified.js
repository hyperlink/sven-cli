	...
	if (utils.isCwdSVN()) {
		...
		utils.promiseSvn('status', cwd, null)
			.then(function(result){
				return massageStatusResult(result).filter(function(entry){
					var attribute = entry['wc-status']['_attribute']
					return !(attribute['file-external'] && attribute.item == 'normal') //exclude unmodified externals
				})
			})
			.then(function displayPretty(entry) {
				if (entry.length) {
					var table = new Table({
						head  : ['Status', 'File', 'Path'].map(function(v){return v.white}),
						style : {compact : true}
					})
				...
				if (entry.length && options.commit) {
					return utils.promisePrompt({
						...
						var commitPackage = {
							...
							files   : entry.filter(isVersioned).map(fileMapper, {cwd: cwd})
						}
...
function isVersioned(value) {
	return value['wc-status']._attribute.item != 'unversioned'
}
